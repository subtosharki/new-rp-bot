    ...
    async execute(interaction: CommandInteraction) {
        if(interaction.options.getSubcommand() === 'text') {
            Phone.find({number: `${interaction.options.getString('number')}`}, 'discordId number', (err, data) => {
                ...
                data.forEach(async (phone) => {
                    if(phone.discordId === interaction.member?.user.id) {
                        ...
                        Text.setAuthor({name: phone.number || 'Unknown Number'});
            ...
            number.save(async (err) => {
                if(err) console.log(err);
                await interaction.reply({content: `Your new phone number is ${number.number}`, ephemeral: true});
            });
        } else {
            Phone.findOneAndRemove(
                { discordId: `${interaction.member?.user.id}` },
            ...
            await interaction.reply({content: `Your phone number has been removed`, ephemeral: true});
        }
    },
};
